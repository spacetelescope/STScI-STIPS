...
from stips.utilities.testing import (verify_data, verify_image)
from stips.astro_image import AstroImage
from stips.observation_module import ObservationModule
from stips import stips_data_base

currentdir = stips_data_base
...
def test_prep_image(in_img,astro_img):
    obs = ObservationModule({})
    obs.prepImage(in_img)
    verify_image(obs.images[in_img['input_image']], astro_img)
    verify_data(obs.images[in_img['input_image']].data, astro_img.data)


@pytest.mark.parametrize(("obs","kwargs","results"), obs_data)
def test_next_observation(obs,kwargs,results):
    ...
    for res in results:
        ...
        if res is None:
            ...
            np.testing.assert_allclose((ra),(om.instrument.ra),atol=1e-3)
            np.testing.assert_allclose((dec),(om.instrument.dec),atol=1e-3)
            np.testing.assert_allclose((pa),(om.instrument.pa),atol=1e-3)
