...
def make_wcs(coords=[0.,0.], coord_types=["RA---TAN", "DEC--TAN"],
             xsize=1, ysize=1, pa=0., scale=[1.,1.], sip=None):
...
def verify_point(dat1,dat2,x,y):
    assert abs(dat1[y,x] - dat2[y,x]) < 1e-3


def verify_data(dat1, dat2):
    ...
    for y in range(dat1.shape[0]):
        for x in range(dat1.shape[1]):
            verify_point(dat1, dat2, x, y)


def verify_image(im1, im2):
...
def verify_parameters(image,results):
